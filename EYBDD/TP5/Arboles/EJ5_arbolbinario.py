...
class BinaryTree:
    ...
    def inordennmm(self):
        def __inorden(root):
            if root is not None:
                __inorden(root.left)
                print(f"{root.value['name']} fue derrotado por {root.value['derrotado']}")
                __inorden(root.right)

        __inorden(self.root)


    def inorden_criaturas(self, criatura):
        def __inorden_c(root, criatura):
            if root is not None:
                __inorden_c(root.left, criatura)
                if root.other_values is criatura:
                    print(root.value)
                __inorden_c(root.right, criatura)
        
        __inorden_c(self.root, criatura)
    ...
    def contar_villians(self):
        def __contar_villians(root):
            count = 0
            if root is not None:
                if root.other_values is False:
                    count = 1
                count += __contar_villians(root.left)
                count += __contar_villians(root.right)
            return count

        return __contar_villians(self.root)
    ...
    def get_heroe_villano(self, arbol1, arbol2):
        
        def __search_s_v(root, arbol1, arbol2):
            if root is not None:
                __search_s_v(root.left, arbol1, arbol2)
                if root.other_values is True:
                    arbol1.insert_node(root.value, root.other_values)
                else:
                    arbol2.insert_node(root.value, root.other_values)
                __search_s_v(root.right, arbol1, arbol2)

        return __search_s_v(self.root, arbol1, arbol2)
